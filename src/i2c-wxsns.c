...
volatile uint8_t tmp117_read_countdown=0;

#define TMP117_I2C_ADDR        (0x48<<1)
...
uint8_t tmp117_read_value(float* tempCelsius)
{
	uint8_t msgBuf[7];
	int16_t tmpVal=0;
	uint8_t fail = 0;
	...
	*tempCelsius = (float)tmpVal * 0.0078125;
...
ISR(TIMER0_COMPA_vect)
{
	...
	if (++ticks >= 10)
	{
		...
		if (tmp117_read_countdown)
			tmp117_read_countdown--;
...
typedef enum
{
	WXSNS_STATE_IDLE          = 0x00,
	WXSNS_STATE_TRIGGER       = 0x10,
	WXSNS_STATE_WAIT          = 0x20,
	WXSNS_STATE_READ          = 0x30,
	WXSNS_STATE_SEND_PACKET   = 0x40,
	WXSNS_STATE_XMIT_WAIT     = 0x50,
	WXSNS_STATE_WAKE          = 0x60,
	WXSNS_STATE_SLEEP         = 0x70
} THState;
...
int main(void)
{
	THState thState = WXSNS_STATE_TRIGGER;
	...
	while (1)
	{
		...
		switch(thState)
		{
			...
			case WXSNS_STATE_WAIT:
				if (isADCDone() && (0 == tmp117_read_countdown) && (0 == sht3x_read_countdown))
					thState = WXSNS_STATE_READ;
				break;

			case WXSNS_STATE_READ:
				{
					float temperature = 0.0;
					float humidity = 0.0;
					float tmp117temperature = 0;
					float16_t k;
					memset(mrbusTxBuffer, 0, sizeof(mrbusTxBuffer));
					tmp117_read_value(&tmp117temperature);
					sht3x_read_value(&temperature, &humidity);
					
					void* ptr = &tmp117temperature;
					uint32_t intFloat = *(uint32_t*)ptr;
			...
			case WXSNS_STATE_SEND_PACKET:
				...
				{
					xbeeIdleCountdown = 3;
					thState = WXSNS_STATE_XMIT_WAIT;
				}
				break;

			case WXSNS_STATE_XMIT_WAIT:
				if (0 == xbeeIdleCountdown)
					thState = WXSNS_STATE_SLEEP;
				break;

			case WXSNS_STATE_WAKE:
				PRR &= ~(_BV(PRTWI) | _BV(PRTIM0) | _BV(PRADC));
				initializeADC();
				enableExternal33();
				i2c_master_init();
				thState = WXSNS_STATE_IDLE;
				break;

			case WXSNS_STATE_SLEEP:
				{
